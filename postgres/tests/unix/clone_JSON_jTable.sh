...
node ../node/jTableImport --username=$DB_USER --hostname=$DB_HOST --password=$DB_PWD file=$MDIR$/$FILENAME.json toUser="$SCHEMA$SCHVER$" logFile=$IMPORTLOG
...
export SCHVER=2
psql -U $DB_USER -h $DB_HOST -a -vSCHEMA=$SCHEMA -vID=$SCHVER -vMETHOD=JSON_TABLE -f sql/RECREATE_SCHEMA.sql >> $LOGDIR$/RECREATE_SCHEMA.log
node ../node/jTableImport --username=$DB_USER --hostname=$DB_HOST --password=$DB_PWD file=$DIR$/${FILENAME}1.json toUser="$SCHEMA$SCHVER$" logFile=$IMPORTLOG
