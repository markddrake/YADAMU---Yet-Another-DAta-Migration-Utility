...
psql -U $DB_USER -h $DB_HOST -a -vID=$SCHVER -vMETHOD=SAX -f sql/RECREATE_ORACLE_ALL.sql >>$LOGDIR$/RECREATE_SCHEMA.log
...
psql -U $DB_USER -h $DB_HOST -a -vID1=1 -vID2=$SCHVER -vMETHOD=SAX -f sql/COMPARE_ORACLE_ALL.sql >>$LOGDIR$/COMPARE_SCHEMA.log
. windows/export_Oracle.bat $DIR $SCHVER $SCHVER $MODE 
