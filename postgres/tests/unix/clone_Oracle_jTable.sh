...
mysql -u$DB_USER -p$DB_PWD -h$DB_HOST -D$DB_DBNAME -P$DB_PORT --init-command="set @ID1=1; set @ID2=$SCHVER; set @METHOD='JSON_TABLE'" --table  <sql/COMPARE_ORACLE_ALL.sql >>$LOGDIR/COMPARE_SCHEMA.log
. windows/export_Oracle.sh $DIR $SCHVER $SCHVER $MODE
