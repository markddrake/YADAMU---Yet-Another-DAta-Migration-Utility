...
-- Note recursive Solutions do not work due to the way Procedures calls are in-lined
--
   ...
   return
     case 
		when INSTR(ISO8601_VALUE,'D') > 0 then
		  SUBSTR(ISO8601_VALUE,1,INSTR(ISO8601_VALUE,'D')-1) || ' Days '
		else
		  ''
     end;
end;
...
BEGIN
   return
     -- Assume a Date Starts with 'P', and DateTime 'PxxxTxxx' and a Time 'PTxxx'
	 -- Assumes the value is a valid ISO8601 Interval String. 
     CASE
        when ISO8601_VALUE is NULL then
	      NULL
  	    when INSTR(ISO8601_VALUE,'T') > 2 then
		  YADAMU.PARSE_ISO8601_YEARS(SUBSTR(ISO8601_VALUE,2,INSTR(ISO8601_VALUE,'T')-1)) || YADAMU.PARSE_ISO8601_HOURS(SUBSTR(ISO8601_VALUE,INSTR(ISO8601_VALUE,'T')+1))
  	    when INSTR(ISO8601_VALUE,'T') > 0 then
		  YADAMU.PARSE_ISO8601_HOURS(SUBSTR(ISO8601_VALUE,3))
		else
