...
*/
--
create or replace function YADAMU_EXPORT(P_SCHEMA VARCHAR, P_SPATIAL_FORMAT VARCHAR, P_OPTIONS SUPER ) 
...
begin
  ...
  loop
    ...
    /*
    **
	** Postgres does not support a length constraint on bytea columns. Most other databases require that a length be specified when creating columns that will manage bytea content.
	** 
    ** If the table contains one or more bytea columns calculate the max length of bytea fields, since Postgres does not maintain a maximum length in the dictionary.
    **
    */
	
    V_SIZE_CONSTRAINTS := r."SIZE_CONSTRAINT_ARRAY";
     
    if (P_OPTIONS ->> 'calculateByteaSize')  then
...
begin
  for r in select "tableName"
                 ,GENERATE_STATEMENTS(P_JSON #>> '{systemInformation,vendor}', P_SCHEMA,"tableName", "columnNames", "dataTypes", "sizeConstraints", P_JSON #>> '{systemInformatio,driverSettings,spatialFormat}', 'jsonb', TRUE) "TABLE_INFO"
...
begin
  ...
  for r in select "tableName"
                 ,GENERATE_STATEMENTS(P_JSON #>> '{systemInformation,vendor}',P_SCHEMA,"tableName","columnNames","dataTypes","sizeConstraints", P_JSON #>> '{systemInformation,driverSettings,spatialFormat}', 'jsonb', FALSE) "TABLE_INFO"
