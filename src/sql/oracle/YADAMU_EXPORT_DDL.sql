...
as
  ...
  procedure RENAME_INDEX(P_TABLE_NAME VARCHAR2, P_INDEXED_COLUMN_LIST VARCHAR2, P_INDEX_NAME VARCHAR2);
...
begin
  ...
  for x in GET_XML_STORAGE(V_OWNER,V_TABLE_NAME) loop
	if (INSTR(P_DDL_STATEMENT,'XMLTYPE COLUMN "' || x.COLUMN_NAME || '"') = 0) then
      case x.STORAGE_TYPE
	    when 'CLOB' then
	      V_STORAGE_CLAUSE := 'CLOB';
	    when 'BINARY' then
	      V_STORAGE_CLAUSE := 'BINARY XML';
	    when 'TRANSPORTABLE BINARY' then
	      V_STORAGE_CLAUSE := 'TRANSPORTABLE BINARY XML';
	    when 'OBJECT-RELATIONAL' then
  	      V_STORAGE_CLAUSE := 'OBJECT RELATIONAL XMLSCHEMA "' || x.XMLSCHEMA || '" ELEMENT "' || x.ELEMENT_NAME || '"';
      end case;
...
as
  ...
  is
   select aic.TABLE_NAME, aic.INDEX_NAME, LISTAGG(COLUMN_NAME,',') WITHIN GROUP (ORDER BY COLUMN_POSITION) INDEXED_COLUMN_LIST
...
--
  ...
  for i in indexedColumnList(P_SCHEMA) loop
    pipe row ('BEGIN YADAMU_EXPORT_DDL.RENAME_INDEX(''' || i.TABLE_NAME  || ''',''' || i.INDEXED_COLUMN_LIST || ''',''' || i.INDEX_NAME || '''); END;');
  end loop;
...
begin
  ...
  where INDEXED_COLUMN_LIST = P_INDEXED_COLUMN_LIST;
...
$IF DBMS_DB_VERSION.VERSION > 21 $THEN 
  -- In 23c Attempting to recreate indexes on AQ's internal IOT tables fails with ORA-24005 so index does not exist
  when NO_DATA_FOUND then
    if (P_TABLE_NAME like 'AQ$_%') then 
      NULL;
	else
	  RAISE;
	end if;
