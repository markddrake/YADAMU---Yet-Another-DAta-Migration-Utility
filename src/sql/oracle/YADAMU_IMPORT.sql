  ...
  select IDX
        ...
        ,'"' || COLUMN_NAME || '" ' ||
         case
           when TYPE_EXISTS = 1 then
             C_MAX_STRING_TYPE
           when TARGET_DATA_TYPE  = '"MDSYS"."SDO_GEOMETRY"' then
             C_MAX_STRING_TYPE
           when TARGET_DATA_TYPE  = 'BOOLEAN' then
             -- Maybe TRUE / FALSE or 01/00
             'NUMBER(1)'
           when TARGET_DATA_TYPE in ('JSON','BFILE') then
           ...
           when TARGET_DATA_TYPE like 'RAW(%)' then
             'VARCHAR2'
           when TARGET_DATA_TYPE in ('CHAR','NCHAR','NVARCHAR2','RAW','ROWID','UROWID') or (TARGET_DATA_TYPE like 'INTERVAL%') then
             'VARCHAR2'
...
exception
  when OTHERS then 
    LOG_INFO(P_COLUMN_NAME_ARRAY);
    LOG_INFO(REPLACE(P_DATA_TYPE_ARRAY,'"."','"."'));
    LOG_INFO(P_SIZE_CONSTRAINT_ARRAY);
	LOG_INFO('{ "CallStack" : "' || YADAMU_UTILITIES.JSON_ESCAPE(DBMS_UTILITY.FORMAT_CALL_STACK()) || '" }');
	LOG_INFO('{ "BackTrace" : "' || YADAMU_UTILITIES.JSON_ESCAPE(DBMS_UTILITY.FORMAT_ERROR_BACKTRACE()) || '" }');
	LOG_INFO('{ "ErrorStack" : "' || YADAMU_UTILITIES.JSON_ESCAPE(DBMS_UTILITY.FORMAT_ERROR_STACK()) || '" }');
...
exception
  when OTHERS then
    -- LOG_INFO(TO_CLOB(P_TYPE_MAPPINGS));
    SET_CURRENT_SCHEMA(V_CURRENT_SCHEMA);
    RAISE;
end;
--
