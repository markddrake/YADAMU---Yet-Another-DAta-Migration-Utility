...
call windows\import_MSSQL_All.bat %MDIR% %SCHVER% ""
sqlcmd -U%DB_USER% -P%DB_PWD% -S%DB_HOST% -d%DB_DBNAME% -I -e -vDATABASE=%DB_DBNAME% -vID1="" -vID2=%SCHVER% -vMETHOD=SAX -i sql\COMPARE_MSSQL_ALL.sql >>%LOGDIR%\COMPARE_SCHEMA.log
...
sqlcmd -U%DB_USER% -P%DB_PWD% -S%DB_HOST% -dAdventureWorks%SCHVER% -I -e -i..\sql\JSON_IMPORT.sql >> %LOGDIR%\install\JSON_IMPORT.log
sqlcmd -U%DB_USER% -P%DB_PWD% -S%DB_HOST% -dAdventureWorksDW%SCHVER% -I -e -i..\sql\JSON_IMPORT.sql >> %LOGDIR%\install\JSON_IMPORT.log
call windows\import_MSSQL_All.bat %DIR% %SCHVER% 1
sqlcmd -U%DB_USER% -P%DB_PWD% -S%DB_HOST% -d%DB_DBNAME% -I -e -vDATABASE=%DB_DBNAME% -vID1=1 -vID2=%SCHVER% -vMETHOD=SAX -i sql\COMPARE_MSSQL_ALL.sql >>%LOGDIR%\COMPARE_SCHEMA.log
call windows\export_MSSQL_All.bat %DIR% %SCHVER% %SCHVER%
