...
@set SCHVER=%~2
@set VER=%~3
node ..\node\jTableImport --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%DB_DBNAME% file=%SRC%\HR%VER%.json toUser=\"HR%SCHVER%\" logFile=%IMPORTLOG% mode=%MODE%
node ..\node\jTableImport --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%DB_DBNAME% file=%SRC%\SH%VER%.json toUser=\"SH%SCHVER%\" logFile=%IMPORTLOG% mode=%MODE%
node ..\node\jTableImport --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%DB_DBNAME% file=%SRC%\OE%VER%.json toUser=\"OE%SCHVER%\" logFile=%IMPORTLOG% mode=%MODE%
