...
exception
  when YADAMU_UTILITIES.JSON_OVERFLOW1 or YADAMU_UTILITIES.JSON_OVERFLOW2 then
    LOG_ERROR(C_FATAL_ERROR,'JSON_TABLE() error. DDL statments larger than ' ||  JSON_FEATURE_DETECTION.C_MAX_STRING_SIZE || ' characters not supported. Use "CLARINET" to process this file.',SQLCODE,SQLERRM,DBMS_UTILITY.FORMAT_ERROR_STACK());
    SET_CURRENT_SCHEMA(V_CURRENT_SCHEMA);
  when OTHERS then
