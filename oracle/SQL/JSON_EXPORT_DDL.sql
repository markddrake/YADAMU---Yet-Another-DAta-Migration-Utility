...
as
  ...
  type T_IMPORT_DDL_LOG_REC is RECORD(
    SQL_STATEMENT   CLOB
   ,SQLCODE         NUMBER
  ...
  TYPE T_IMPORT_DDL_LOG_TAB is TABLE of T_IMPORT_DDL_LOG_REC;

  IMPORT_DDL_LOG_TABLE T_IMPORT_DDL_LOG_TAB := T_IMPORT_DDL_LOG_TAB();
  ...
  function FETCH_DDL_STATEMENTS(P_SCHEMA VARCHAR2) return T_CLOB_TAB pipelined;
  procedure APPLY_DDL_STATEMENTS(P_EXPORT_CONTENTS IN OUT NOCOPY CLOB, P_TARGET_SCHEMA VARCHAR2 DEFAULT SYS_CONTEXT('USERENV','CURRENT_SCHEMA'));
  function IMPORT_DDL_LOG return T_IMPORT_DDL_LOG_TAB pipelined;
...
return T_IMPORT_DDL_LOG_TAB
pipelined
as
  cursor getRecords
  is
  select *
    from TABLE(IMPORT_DDL_LOG_TABLE);
begin
...
--
PROCEDURE LOG_OPERATION(P_SQL_STATEMENT CLOB, P_SQLCODE NUMBER, P_RESULT VARCHAR2, P_STATUS VARCHAR2)
as
begin
  IMPORT_DDL_LOG_TABLE.extend();
  IMPORT_DDL_LOG_TABLE(IMPORT_DDL_LOG_TABLE.count).SQL_STATEMENT := P_SQL_STATEMENT;
  IMPORT_DDL_LOG_TABLE(IMPORT_DDL_LOG_TABLE.count).SQLCODE := P_SQLCODE;
  IMPORT_DDL_LOG_TABLE(IMPORT_DDL_LOG_TABLE.count).LOGENTRY := P_RESULT;
  IMPORT_DDL_LOG_TABLE(IMPORT_DDL_LOG_TABLE.count).STATUS  := P_STATUS;
end;
--
...
as
  ...
  FUNCTION_MV_HACK CONSTANT VARCHAR2(512) := 'create or replace procedure FIX_MV_PERMSISSIONS_ISSUE(P_BASE_TABLE VARCHAR2)' || C_NEWLINE
                                          || 'as' || C_NEWLINE
										  || 'begin' || C_NEWLINE
...
begin
  ...
  V_SQL_STATEMENT := FUNCTION_MV_HACK;
  execute immediate V_SQL_STATEMENT;
  $IF $$DEBUG $THEN LOG_OPERATION(V_SQL_STATEMENT,0,NULL,C_SUCCESS); $END
  begin 
    V_SQL_STATEMENT := 'begin FIX_MV_PERMSISSIONS_ISSUE(''' || V_BASE_TABLE_NAME || '''); end;';
    execute immediate V_SQL_STATEMENT;
    $IF $$DEBUG $THEN LOG_OPERATION(V_SQL_STATEMENT,0,NULL,C_SUCCESS); $END  
  exception
    when INHERIT_PRIVS_ISSUE then
	  ...
	  exception 
        when OTHERS then 
		  LOG_OPERATION(V_SQL_STATEMENT,SQLCODE,JSON_OBJECT(C_FATAL_ERROR value JSON_OBJECT('sql' value V_SQL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_FATAL_ERROR);  
	  end;
...
begin
  IMPORT_DDL_LOG_TABLE := T_IMPORT_DDL_LOG_TAB();
  SET_CURRENT_SCHEMA(P_TARGET_SCHEMA);
  if (IMPORT_DDL_LOG_TABLE.count = 0) then
    for s in getDDLStatements loop
	  ...
	  begin
        ...
        $IF $$DEBUG $THEN LOG_OPERATION(V_DDL_STATEMENT,0,NULL,C_SUCCESS); $END  
	  exception
		when DUPLICATE_OBJECT or DUPLICATE_INDEX or DUPLICATE_PKEY or DUPLICATE_KEY or DUPLICATE_NAME or DUPLICATE_REF_CONSTRAINT
  		  then
			LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_DUPLICATE value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_DUPLICATE);  
	    when COMPILATION_ERROR
		  then
			LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_WARNING value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_WARNING);  
	    when NO_TABLESPACE_PRIVILEGES
		  then
		    if (upper(V_DDL_STATEMENT) LIKE '%USAGE QUEUE') then
  			  LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_AQ_ISSUE value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_AQ_ISSUE);  
			else
  			  LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_FATAL_ERROR value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_FATAL_ERROR);  
	        end if;
	    when NO_DATA_FOUND
		  then
	    ...
	    when MISSING_OBJECT
		     then
    		   LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_REFERENCE value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_REFERENCE);  
		when OTHERS
		  then
  			LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,JSON_OBJECT(C_FATAL_ERROR value JSON_OBJECT('sql' value V_DDL_STATEMENT, 'stack' value DBMS_UTILITY.format_error_stack)),C_FATAL_ERROR);  
	    end;
