...
begin
  ...
  select JSON_OBJECT('trace' value JSON_OBJECT('ddlStatement' value P_SQL_STATEMENT
...
begin
  RESULTS_CACHE.extend();
  select JSON_OBJECT('error' value JSON_OBJECT('severity' value P_SEVERITY, 'code' value P_SQLCODE, 'msg' value P_SQLERRM, 'sqlStatement' value P_SQL_STATEMENT, 'stack' value P_STACK
                     $IF JSON_FEATURE_DETECTION.CLOB_SUPPORTED $THEN
                     returning CLOB) returning CLOB)
                     $ELSIF JSON_FEATURE_DETECTION.EXTENDED_STRING_SUPPORTED $THEN
                     returning  VARCHAR2(32767)) returning  VARCHAR2(32767))
...
begin
  ...
  for s in getDDLStatements loop
    ...
    begin
      ...
      $IF $$DEBUG $THEN TRACE_OPERATION(V_DDL_STATEMENT); $END
    exception
      ...
      when COMPILATION_ERROR
        then
          LOG_ERROR(C_WARNING,V_DDL_STATEMENT,SQLCODE,SQLERRM,DBMS_UTILITY.FORMAT_ERROR_STACK());
