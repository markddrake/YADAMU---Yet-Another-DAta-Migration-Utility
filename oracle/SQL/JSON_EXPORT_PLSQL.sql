--
create or replace package JSON_EXPORT_PLSQL
authid CURRENT_USER
as
...
show errors 
--
create or replace package body JSON_EXPORT_PLSQL
as
--
  ...
  begin
    ...
    for t in getTableMetadata loop
	  ...
	  select JSON_OBJECT(
              ...
              ,'columns'                     value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.COLUMN_LIST)
              ,'dataTypes'                   value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.DATA_TYPE_LIST)
			  ,'dataTypeSizing'              value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.SIZE_CONSTRAINT_LIST)
              ,'exportSelectList'            value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.EXPORT_SELECT_LIST)
              ,'insertSelectList'            value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.IMPORT_SELECT_LIST)
              ,'columnPatterns'              value JSON_EXPORT_PLSQL.SERIALIZE_TABLE(t.COLUMN_PATTERN_LIST)
                returning CLOB
             )
	...
	APPEND_SERIALIZATION_FUNCTIONS(V_OBJECT_SERIALIZATION,V_BFILE_COUNT,V_BLOB_COUNT,V_ANYDATA_COUNT,SQL_STATEMENT);
    ...
    V_SQL_FRAGMENT := 'select JSON_OBJECT(
                                ''systemInformation'' value JSON_OBJECT(
		                                                     ...
		                                                     ,''exportVersion''   value JSON_EXPORT_PLSQL.EXPORT_VERSION()
 		                                                     ,''jsonFeatures''    value ' ||
				   				  	                          $IF JSON_FEATURE_DETECTION.TREAT_AS_JSON_SUPPORTED $THEN
		                                                         'treat(JSON_EXPORT_PLSQL.JSON_FEATURES() as JSON)'
					  				                          $ELSE
		                                                         'JSON_QUERY(JSON_EXPORT_PLSQL.JSON_FEATURES(),''$'')'
 						  			                          $END || '
	                                                         ,''sessionUser''     value SYS_CONTEXT(''USERENV'',''SESSION_USER'')
		                                                     ,''dbName''          value SYS_CONTEXT(''USERENV'',''DB_NAME'')
		                                                     ,''''  value SYS_CONTEXT(''USERENV'',''SERVER_HOST'')
		                                                     ,''databaseVersion'' value JSON_EXPORT_PLSQL.DATABASE_RELEASE()
		                                                     ,''nlsInformation''  value (select JSON_OBJECTAGG(parameter, value) from NLS_DATABASE_PARAMETERS)
	                                                         )';

    if (G_INCLUDE_DDL) then
      V_SQL_FRAGMENT := V_SQL_FRAGMENT
	                 || ',''ddl'' value (select JSON_ARRAYAGG(COLUMN_VALUE returning CLOB) from TABLE(JSON_EXPORT_PLSQL_DDL.FETCH_DDL_STATEMENTS(:DDL_SCHEMA)))';
	end if;
...
--
  ...
  begin
    ...
    for t in getTableMetadata loop
	  ...
	  APPEND_DESERIALIZATION_FUNCTIONS(V_OBJECT_SERIALIZATION,t.BFILE_COUNT,t.BLOB_COUNT,t.ANYDATA_COUNT,V_SQL_STATEMENT);
      begin
...
begin
  select JSON_OBJECT(
		   ...
		   value JSON_OBJECT(
	              ...
	              ,'schema'          value P_OWNER_LIST
		          ,'exportVersion'   value JSON_EXPORT_PLSQL.EXPORT_VERSION()
		          ,'jsonFeatures'    value JSON_QUERY(JSON_EXPORT_PLSQL.JSON_FEATURES(),'$')
	              ,'sessionUser'     value SYS_CONTEXT('USERENV','SESSION_USER')
		          ,'dbName'          value SYS_CONTEXT('USERENV','DB_NAME')
