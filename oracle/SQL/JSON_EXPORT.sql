...
as
  ...
  select aat.owner
		...
		,sum(case when ((TYPECODE in ('COLLECTION', 'OBJECT')) and (DATA_TYPE not in ('XMLTYPE','ANYDATA','RAW'))) then 1 else 0 end) OBJECT_COUNT
        ,cast(collect('"' || COLUMN_NAME || '"' ORDER BY INTERNAL_COLUMN_ID) as T_VC4000_TABLE) COLUMN_LIST
		,cast(collect(
                case 
                  when (DATA_TYPE_OWNER is null) 
                    then '"' || DATA_TYPE || '"' 
                  when (DATA_TYPE in ('XMLTYPE','ANYDATA','RAW'))
                    then '"' || DATA_TYPE || '"' 
                  else 
