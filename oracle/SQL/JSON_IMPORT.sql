...
--  
  ...
  function GENERATE_COLUMN_PATTERNS(P_COLUMN_NAMES CLOB, P_DATA_TYPES CLOB) return CLOB;
...
--
function LIST_TO_TABLE(P_LIST CLOB)
return T_VC4000_TABLE
as
  V_TABLE T_VC4000_TABLE;
begin
  select cast(collect(x.COLUMN_VALUE.getStringVal()) as T_VC4000_TABLE) IMPORT_SELECT_LIST
   into V_TABLE 
...
--
function GENERATE_COLUMN_PATTERNS(P_COLUMN_NAMES CLOB, P_DATA_TYPES CLOB)
return CLOB
as
  V_COLUMN_PATTERNS   CLOB;
  V_COLUMN_PATTERN    VARCHAR2(4000);
  V_COLUMN_NAME_TABLE T_VC4000_TABLE;
  V_DATA_TYPE_TABLE   T_VC4000_TABLE;
begin
  V_COLUMN_NAME_TABLE := LIST_TO_TABLE(P_COLUMN_NAMES);
  V_DATA_TYPE_TABLE   := LIST_TO_TABLE(P_DATA_TYPES);
  DBMS_LOB.CREATETEMPORARY(V_COLUMN_PATTERNS,TRUE,DBMS_LOB.CALL);  
  for i in 1..V_COLUMN_NAME_TABLE.count loop
	if (i = 1) then
	  V_COLUMN_PATTERN := '       ';
    else
	  V_COLUMN_PATTERN := C_NEWLINE  || '      ,';
    end if;
    V_COLUMN_PATTERN := V_COLUMN_PATTERN || '"' ||  V_COLUMN_NAME_TABLE(i) || '" ' || V_DATA_TYPE_TABLE(i) || ' PATH ''$[' || (i-1) || ']''' || CHR(13);
	DBMS_LOB.WRITEAPPEND(V_COLUMN_PATTERNS,LENGTH(V_COLUMN_PATTERN),V_COLUMN_PATTERN);
  end loop;
...
begin
  select OWNER
	    ...
	    'into "' || TABLE_NAME ||'"(' || SELECT_LIST || ')' || C_NEWLINE ||
		 ...
		 '         COLUMNS(' || C_NEWLINE ||  GENERATE_COLUMN_PATTERNS(SELECT_LIST,COLUMN_PATTERNS) || C_NEWLINE || '))' 
	    ,NULL
	    ,NULL
		,NULL
	bulk collect into V_SQL_OPERATIONS
