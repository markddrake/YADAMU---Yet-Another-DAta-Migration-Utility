...
:windowsContainer
  ...
  @set LINUX_HOST_LIST=--add-host="VRTCA09-01:%LINUX_HOST%" --add-host="VRTCA10-01:%LINUX_HOST%" --add-host="VRTCA11-01:%LINUX_HOST%" --add-host="MINIO-01:%LINUX_HOST%" --add-host="AZURE-01:%LINUX_HOST%"
  docker run --name %CONTAINER_NAME% --network YADAMU-NET %LINUX_HOST_LIST% --memory=%MAX_MEMORY% -v C:\SHARED:C:\YADAMU\mnt -e YADAMU_TEST_NAME=%YADAMU_TEST_NAME% -e TESTNAME=%CONFIGURATION_FILE% -d %TARGET_IMAGE%
  exit /b
:end
