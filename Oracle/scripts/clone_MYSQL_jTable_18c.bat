...
node node\jTableImport userid=SYSTEM/oracle@%TNS% File=%MDIR%\%FILENAME%.json toUser=%SCHEMA%%ID%
node node\export userid=SYSTEM/oracle@%TNS% File=%DIR%\%FILENAME%%ID%.json owner=%SCHEMA%%ID%
sqlplus system/oracle@%TNS% @TESTS/RECREATE_SCHEMA.sql %SCHEMA% %ID%
node node\jTableImport userid=SYSTEM/oracle@%TNS% File=%DIR%\%FILENAME%1.json touser=%SCHEMA%%ID%
node node\export userid=SYSTEM/oracle@%TNS% File=%DIR%\%FILENAME%%ID%.json owner=%SCHEMA%%ID%
dir %DIR%\*1.json
dir %DIR%\*2.json
