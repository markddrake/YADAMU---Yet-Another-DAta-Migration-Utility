...
**  VERSION 4
**
**  Add support for BFILE and BLOB using a PL/SQL procedure and a WITH Clause
**
*/
...
as
  C_FUNCTION  CONSTANT VARCHAR2(512)   := 'function BFILE2CHAR(P_BFILE BFILE) return VARCHAR2' || C_NEWLINE
                                       || 'as' || C_NEWLINE
                                       || '  C_SINGLE_QUOTE     CONSTANT CHAR(1) := CHR(39);' || C_NEWLINE
                                       ...
                                       || '  return ''BFILENAME('' || C_SINGLE_QUOTE || V_DIRECTORY_ALIAS || C_SINGLE_QUOTE || '','' || C_SINGLE_QUOTE || V_PATH2FILE || C_SINGLE_QUOTE || '')'';' || C_NEWLINE
                                       || 'end;' || C_NEWLINE;
begin
  return C_FUNCTION;
end;
...
as
  C_FUNCTION  CONSTANT VARCHAR2(1024)  := 'function BLOB2HEXBINARY(P_BLOB BLOB)' || C_NEWLINE
                                       ...
                                       || '  V_HEXBINARY_DATA VARCHAR2(32767);' || C_NEWLINE
                                       || 'begin' || C_NEWLINE
                                       ...
                                       || '    V_HEXBINARY_DATA := RAWTOHEX(V_RAW_DATA);' || C_NEWLINE
                                       || '    DBMS_LOB.WRITEAPPEND(V_CLOB,LENGTH(V_HEXBINARY_DATA),V_HEXBINARY_DATA);' || C_NEWLINE
                                       || '  end loop;' || C_NEWLINE
  ...
  select aat.owner
	    ...
	    ,cast(collect(
			   case
				 ...
				 **
				 */
			   	 when DATA_TYPE in ('LONG','LONG RAW','ANYDATA')
			       then '''"' || COLUMN_NAME || '". Unsupported data type ["' || DATA_TYPE || '"]'''
				 when TYPECODE = 'COLLECTION'
			       then '''"' || COLUMN_NAME || '". Unsupported collection ["' || DATA_TYPE_OWNER || '"."' || DATA_TYPE || '"]'''
				 when TYPECODE = 'OBJECT'
			       then '''"' || COLUMN_NAME || '". Unsupported object ["' || DATA_TYPE_OWNER || '"."' || DATA_TYPE || '"]'''
			     else
   			       '"' || COLUMN_NAME || '"'
		       end
   ...
   where aat.STATUS = 'VALID'
	 ...
	 **
	 */
	 and ((atc.HIDDEN_COLUMN = 'NO') and ((atc.VIRTUAL_COLUMN = 'NO') or ((atc.VIRTUAL_COLUMN = 'YES') and (atc.DATA_TYPE = 'XMLTYPE'))))
...
begin
  ...
  for t in getTableMetadata loop
    ...
    if ((t.BFILE_COUNT > 0) or  (t.BLOB_COUNT > 0)) then
	  V_SQL_FRAGMENT := 'WITH' ||  C_NEWLINE;
      DBMS_LOB.WRITEAPPEND(V_SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
	  if (t.BFILE_COUNT > 0) then
        V_SQL_FRAGMENT := FUNCTION_BFILE2CHAR;
	    DBMS_LOB.WRITEAPPEND(V_SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
      end if;
	  if (t.BLOB_COUNT > 0) then
        V_SQL_FRAGMENT := FUNCTION_BLOB2HEXBINARY;
	    DBMS_LOB.WRITEAPPEND(V_SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
	  end if;
	end if;
  ...
  if ((V_BFILE_COUNT > 0) or  (V_BLOB_COUNT > 0)) then
    V_SQL_FRAGMENT := 'WITH' ||  C_NEWLINE;
    DBMS_LOB.WRITEAPPEND(SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
	if (V_BFILE_COUNT > 0) then
      V_SQL_FRAGMENT := FUNCTION_BFILE2CHAR;
	  DBMS_LOB.WRITEAPPEND(SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
    end if;
	if (V_BLOB_COUNT > 0) then
      V_SQL_FRAGMENT := FUNCTION_BLOB2HEXBINARY;
	  DBMS_LOB.WRITEAPPEND(SQL_STATEMENT,LENGTH(V_SQL_FRAGMENT),V_SQL_FRAGMENT);
	end if;
  end if;
