...
as
  ...
  type T_ERROR_LOG_REC is RECORD(
    SQL_STATEMENT   CLOB
   ,SQLCODE         NUMBER
  ...
  TYPE T_ERROR_LOG_TAB is TABLE of T_ERROR_LOG_REC;

  ERROR_LOG_TABLE T_ERROR_LOG_TAB := T_ERROR_LOG_TAB();
  ...
  function ERROR_LOG return T_ERROR_LOG_TAB pipelined;
...
--
function ERROR_LOG
return T_ERROR_LOG_TAB
pipelined
as
  cursor getRecords
  is
  select *
    from TABLE(ERROR_LOG_TABLE);
begin
...
as
begin
  ERROR_LOG_TABLE.extend();
  ERROR_LOG_TABLE(ERROR_LOG_TABLE.count).SQL_STATEMENT := P_SQL_STATEMENT;
  ERROR_LOG_TABLE(ERROR_LOG_TABLE.count).SQLCODE := P_SQLCODE;
  ERROR_LOG_TABLE(ERROR_LOG_TABLE.count).RESULT := P_RESULT;
  ERROR_LOG_TABLE(ERROR_LOG_TABLE.count).STATUS  := P_STATUS;
end;
--
...
begin
  ERROR_LOG_TABLE := T_ERROR_LOG_TAB();
  SET_CURRENT_SCHEMA(P_TARGET_SCHEMA);
  if (ERROR_LOG_TABLE.count = 0) then
