...
as
  ...
  type T_EXPORT_DDL_LOG_REC is RECORD(
    SQL_STATEMENT   CLOB
   ,SQLCODE         NUMBER
  ...
  TYPE T_EXPORT_DDL_LOG_TAB is TABLE of T_EXPORT_DDL_LOG_REC;

  EXPORT_DDL_LOG_TABLE T_EXPORT_DDL_LOG_TAB := T_EXPORT_DDL_LOG_TAB();
  ...
  function EXPORT_DDL_LOG return T_EXPORT_DDL_LOG_TAB pipelined;
  PROCEDURE RENAME_INDEX(P_TABLE_NAME VARCHAR2, P_COLUMN_LIST VARCHAR2, P_INDEX_NAME VARCHAR2);
...
--
function EXPORT_DDL_LOG
return T_EXPORT_DDL_LOG_TAB
pipelined
as
  cursor getRecords
  is
  select *
    from TABLE(EXPORT_DDL_LOG_TABLE);
begin
...
as
begin
  EXPORT_DDL_LOG_TABLE.extend();
  EXPORT_DDL_LOG_TABLE(EXPORT_DDL_LOG_TABLE.count).SQL_STATEMENT := P_SQL_STATEMENT;
  EXPORT_DDL_LOG_TABLE(EXPORT_DDL_LOG_TABLE.count).SQLCODE := P_SQLCODE;
  EXPORT_DDL_LOG_TABLE(EXPORT_DDL_LOG_TABLE.count).RESULT := P_RESULT;
  EXPORT_DDL_LOG_TABLE(EXPORT_DDL_LOG_TABLE.count).STATUS  := P_STATUS;
end;
--
...
begin
  ...
  exception
    when INHERIT_PRIVS_ISSUE then
	  ...
	  exception 
        when OTHERS then 
		  LOG_OPERATION(V_SQL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_FATAL_ERROR);  
	  end;
...
begin
  ...
  where INDEXED_COLUMN_LIST = P_COLUMN_LIST;
...
begin
  EXPORT_DDL_LOG_TABLE := T_EXPORT_DDL_LOG_TAB();
  SET_CURRENT_SCHEMA(P_TARGET_SCHEMA);
  if (EXPORT_DDL_LOG_TABLE.count = 0) then
    for s in getDDLStatements loop
	  ...
	  exception
		when DUPLICATE_OBJECT or DUPLICATE_INDEX or DUPLICATE_PKEY or DUPLICATE_KEY or DUPLICATE_NAME or DUPLICATE_REF_CONSTRAINT
  		  then
            LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_DUPLICATE);  
	    when COMPILATION_ERROR
		  then
            LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_WARNING);  
	    when NO_TABLESPACE_PRIVILEGES
		  then
		    if (upper(V_DDL_STATEMENT) LIKE '%USAGE QUEUE') then
			  LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_AQ_ISSUE);  
			else
	          LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_FATAL_ERROR);  
	        end if;
	    when NO_DATA_FOUND
		  then
	    ...
	    when MISSING_OBJECT
		     then
               LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_REFERENCE);  
		when OTHERS
		  then
            LOG_OPERATION(V_DDL_STATEMENT,SQLCODE,DBMS_UTILITY.format_error_stack,C_FATAL_ERROR);  
	    end;
