...
--
/*
** Use WITH clause to add PL/SQL based workarounds for BLOB and BFILE  
*/																						   
...
return CLOB
is
  MAX_SUPPORTED_SIZE CONSTANT NUMBER := ' || (TRUNC(C_MAX_OUTPUT_SIZE / 2) - 4) || ';
...
begin
  if (V_BLOB_SIZE > MAX_SUPPORTED_SIZE) then
    return TO_CLOB(''BLOB2HEXBINARY: Input size ('' || V_BLOB_SIZE || '') exceeds maximum supported size ('' || MAX_SUPPORTED_SIZE || '').'');
  end if;
...
--
procedure GENERATE_WITH_CLAUSE(P_BFILE_COUNT NUMBER, P_BLOB_COUNT NUMBER, P_SQL_STATEMENT IN OUT CLOB)
as
begin
			   ...
			   case
				 ...
				 */
				 ...
				 when DATA_TYPE_OWNER is not NULL
			       then '''"' || COLUMN_NAME || '". Unsupported type ["' || DATA_TYPE_OWNER || '"."' || DATA_TYPE || '"]'''
			     else
   			       '"' || COLUMN_NAME || '"'
		       end
		order by COLUMN_ID) as T_VC4000_TABLE) EXPORT_SELECT_LIST
...
--
  ...
  declare
   ...
   begin 
    ...
    GENERATE_WITH_CLAUSE(V_BFILE_COUNT,V_BLOB_COUNT,SQL_STATEMENT);
  end;
...
--
  ...
  begin
    ...
    for t in getTableMetadata loop  
      ...
      GENERATE_WITH_CLAUSE(t.BFILE_COUNT,t.BLOB_COUNT,V_SQL_STATEMENT); 
