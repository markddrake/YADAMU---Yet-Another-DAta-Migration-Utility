...
--
DROP FUNCTION IF EXISTS MAP_ORACLE_DATATYPES;
--
DELIMITER $$
--
CREATE FUNCTION MAP_ORACLE_DATATYPES(P_DATA_TYPE VARCHAR(128), P_SIZE_CONSTRAINT VARCHAR(128)) 
...
BEGIN
  ...
  WHILE I < JSON_LENGTH(V_COLUMN_ARRAY) DO
    ...
    SET V_COLUMN_PATTERNS = CONCAT(V_COLUMN_PATTERNS,V_COLUMN_PATTERN,'"',JSON_UNQUOTE(JSON_EXTRACT(V_COLUMN_ARRAY,V_INDEX)),'" ',MAP_ORACLE_DATATYPES(JSON_UNQUOTE(JSON_EXTRACT(V_DATA_TYPE_ARRAY,V_INDEX)),JSON_UNQUOTE(JSON_EXTRACT(V_DATA_SIZE_ARRAY,V_INDEX))),' PATH ''',V_INDEX,'''');
    SET I = I + 1;
  END WHILE;
...
BEGIN
  ...
  WHILE I < JSON_LENGTH(V_COLUMN_ARRAY) DO
    ...
    SET V_INDEX := CONCAT('$[',I,']');
    SET V_COLUMN_PATTERNS = CONCAT(V_COLUMN_PATTERNS,V_COLUMN_PATTERN,'"',JSON_UNQUOTE(JSON_EXTRACT(V_COLUMN_ARRAY,V_INDEX)),'" ',MAP_ORACLE_DATATYPES(JSON_UNQUOTE(JSON_EXTRACT(V_DATA_TYPE_ARRAY,V_INDEX)),JSON_UNQUOTE(JSON_EXTRACT(V_DATA_SIZE_ARRAY,V_INDEX))));
    SET I = I + 1;
  END WHILE;
...
DELIMITER $$
--
CREATE PROCEDURE IMPORT_JSON(P_TARGET_DATABASE VARCHAR(128)) 
BEGIN
  ...
  DELETE FROM IMPORT_LOGGING;
  set @SQL_STATEMENT = 'INSERT INTO "IMPORT_LOGGING" values (?)';
  PREPARE LOG_STATEMENT FROM @SQL_STATEMENT;
  ...
  PROCESS_TABLE : LOOP
	...
	/*
    SET @LOG_RECORD = V_DDL_STATEMENT;
    EXECUTE LOG_STATEMENT USING @LOG_RECORD;
    SET @LOG_RECORD = V_DML_STATEMENT;
    EXECUTE LOG_STATEMENT USING @LOG_RECORD;
	*/
	
	SET @SQL_STATEMENT = V_DDL_STATEMENT;
    PREPARE STATEMENT FROM @SQL_STATEMENT;
    EXECUTE STATEMENT;
    DEALLOCATE PREPARE STATEMENT;
    
	SET @SQL_STATEMENT = V_DML_STATEMENT;
    PREPARE STATEMENT FROM @SQL_STATEMENT;
	...
	SET V_LOG_ENTRY = JSON_OBJECT('tableName', V_TABLE_NAME,'rowCount', V_ROW_COUNT,'elapsedTime', V_ELAPSED_TIME);
	SET @LOG_RECORD = V_LOG_ENTRY;
    EXECUTE LOG_STATEMENT USING @LOG_RECORD;
    -- 
  END LOOP;
