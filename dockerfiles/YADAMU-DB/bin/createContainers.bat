...
docker run --name ORA1903-01 -p 1521:1521 -e ORACLE_SID=CDB19300 -e ORACLE_PDB=PDB19300 -e ORACLE_PWD=oracle -e ORACLE_CHARACTERSET=AL32UTF8 -vORA1903-01-DATA:/opt/oracle/oradata -vORA1903-01-DIAG:/opt/oracle/diag -vORA1903-01-DIAG:/opt/oracle/admin --shm-size=4g --tmpfs /dev/shm/:rw,nosuid,nodev,exec,size=4g -d oracle/database:19.3.0-ee
REM Oracle 18.3.0.0.0                                                                                                                                                        
docker run --name ORA1803-01 -p 1522:1521 -e ORACLE_SID=CDB18300 -e ORACLE_PDB=PDB18300 -e ORACLE_PWD=oracle -e ORACLE_CHARACTERSET=AL32UTF8 -vORA1803-01-DATA:/opt/oracle/oradata -vORA1803-01-DIAG:/opt/oracle/diag -vORA1803-01-DIAG:/opt/oracle/admin --shm-size=4g --tmpfs /dev/shm/:rw,nosuid,nodev,exec,size=4g -d oracle/database:18.3.0-ee
...
docker run --name ORA1120-01 -p 1525:1521 -e ORACLE_SID=ORA11200 -e ORACLE_PDB=ORA11200 -e ORACLE_PWD=oracle -e ORACLE_CHARACTERSET=AL32UTF8 -vORA1120-01-DATA:/opt/oracle/oradata -vORA1120-01-DIAG:/opt/oracle/diag -vORA1120-01-DIAG:/opt/oracle/admin  --shm-size=4g --tmpfs /dev/shm/:rw,nosuid,nodev,exec,size=4g -d oracle/database:11.2.0.1-ee
REM Oracle MySQL 8.0
docker run --name MYSQL80-01 -p 3306:3306  -v MYSQL80-01-DATA:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=oracle  --shm-size=4g -d mysql:latest 
REM MaraDB 10.0
