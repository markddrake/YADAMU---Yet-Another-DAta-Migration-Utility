...
node $YADAMU_DB_ROOT/node/import --username=$DB_USER --hostname=$DB_HOST --password=$DB_PWD --database=$DB_DBNAME file=$SRC/AdventureWorksDW$FILEVER.json toUser=\"DW$SCHEMAVER\"             logFile=$IMPORTLOG mode=$MODE
