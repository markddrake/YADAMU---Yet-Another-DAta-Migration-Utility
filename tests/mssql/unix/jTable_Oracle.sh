...
export SCHEMAVER=1
sqlcmd -U$DB_USER -P$DB_PWD -S$DB_HOST -d$DB_NAME -I -e -vID=$SCHEMAVER -i$YADAMU_SCRIPT_ROOT/sql/RECREATE_ORACLE_ALL.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
...
sqlcmd -U$DB_USER -P$DB_PWD -S$DB_HOST -d$DB_NAME -I -e -vDATABASE=$DB_NAME -vID1=1 -vID2=$SCHEMAVER -vMETHOD=$YADAMU_PARSER -i$YADAMU_SCRIPT_ROOT/sql/COMPARE_ORACLE_ALL.sql >>$YADAMU_LOG_PATH/COMPARE_SCHEMA.log
. $YADAMU_SCRIPT_ROOT/unix/export_Oracle.sh $YADAMU_OUTPUT_PATH $SCHEMAVER $SCHEMAVER $MODE 
