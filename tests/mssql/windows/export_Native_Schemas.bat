...
@set SCHEMA=ADVWRK
sqlcmd -U%DB_USER% -P%DB_PWD% -S%DB_HOST% -d%DB_DBNAME% -I -e -vMSSQL_SCHEMA=%SCHEMA% -i%YADAMU_SCRIPT_ROOT%\sql\RECREATE_SCHEMA.sql >>%YADAMU_LOG_PATH%\RECREATE_SCHEMA.log
call %YADAMU_SCRIPT_ROOT%\windows\import_MSSQL_ALL.bat %YADAMU_INPUT_PATH% "" "" 
node %YADAMU_DB_ROOT%\node\export --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%SCHEMA% file=%YADAMU_INPUT_PATH%\%FILENAME%.json owner=\"dbo\" mode=%MODE% log_file=%EXPORTLOG%