...
mysql -u$DB_USER -p$DB_PWD -h$DB_HOST -D$DB_DBNAME -P$DB_PORT -v -f <$YADAMU_HOME/mysql/sql/JSON_IMPORT.sql >$YADAMU_LOG_PATH/JSON_IMPORT_MYSQL.log
source $YADAMU_HOME/tests/mariadb/env/dbConnection.sh
mysql -u$DB_USER -p$DB_PWD -h$DB_HOST -D$DB_DBNAME -P$DB_PORT -v -f <$YADAMU_HOME/mariadb/sql/JSON_IMPORT.sql >$YADAMU_LOG_PATH/JSON_IMPORT_MARIADB.log
