...
sh $YADAMU_SCRIPT_ROOT/unix/jTableImport_MSSQL.sh $YADAMU_INPUT_PATH $SCHEMAVER ""
sh $YADAMU_SCRIPT_ROOT/unix/export_MSSQL.sh $YADAMU_OUTPUT_PATH $SCHEMAVER $SCHEMAVER
export SCHEMAVER=2
mysql -u$DB_USER -p$DB_PWD -h$DB_HOST -D$DB_DBNAME -P$DB_PORT -v -f --init-command="export @ID=$SCHEMAVER; then export @METHOD='$YADAMU_PARSER';" <$YADAMU_SCRIPT_ROOT/sql/RECREATE_MSSQL_ALL.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
sh $YADAMU_SCRIPT_ROOT/unix/jTableImport_MSSQL.sh $YADAMU_OUTPUT_PATH $SCHEMAVER 1
