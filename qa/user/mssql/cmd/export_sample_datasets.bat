...
call %YADAMU_SCRIPT_PATH%\export_operations_MsSQL.bat %YADAMU_OUTPUT_PATH% "" "" %MODE%
...
node %YADAMU_BIN%\import --rdbms=%YADAMU_DB% --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%SCHEMA% file=%YADAMU_OUTPUT_PATH%\%FILENAME%.json overwrite=yes from_user=\"dbo\" mode=%MODE% log_file=%YADAMU_EXPORT_LOG%