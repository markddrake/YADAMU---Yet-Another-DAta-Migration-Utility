create or replace procedure YADAMU_SYSTEM.PUBLIC.COMPARE_SCHEMAS(P_DATABASE STRING,P_SOURCE_SCHEMA STRING, P_TARGET_SCHEMA STRING, P_EMPTY_STRING_IS_NULL BOOLEAN, P_TIMESTAMP_PRECISION FLOAT)
returns VARIANT
language javascript
as
$$
const timeStampLength = 20 + P_TIMESTAMP_PRECISION;
...
select table_name, listagg(
                     case 
                       ...
                       ${P_EMPTY_STRING_IS_NULL === 1 ? `when data_type in ('TEXT','VARCHAR') then 'case when "' || column_name || '" = '''' then NULL else "' || column_name || '" end "' || column_name || '"'` : ''}
                       else 
                         '"' || column_name || '"'
                     end
					,','
