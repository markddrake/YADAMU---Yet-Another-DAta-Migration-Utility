...
call %YADAMU_BIN%\upload.bat --RDBMS=%YADAMU_VENDOR% --USERNAME=%DB_USER% --HOSTNAME=%DB_HOST% --PORT=%DB_PORT% --PASSWORD=%DB_PWD% --DATABASE=%DB_DBNAME%  FILE=%YADAMU_IMPORT_MSSQL%\%FILENAME%.json TO_USER=\"%SCHEMA%%SCHEMA_VERSION%\"  MODE=%MODE%  LOG_FILE=%YADAMU_IMPORT_LOG% EXCEPTION_FOLDER=%YADAMU_LOG_PATH%
...
set /a SCHEMA_VERSION+=1
mysql -u%DB_USER% -p%DB_PWD% -h%DB_HOST% -D%DB_DBNAME% -P%DB_PORT%  -v -f --init-command="set @SCHEMA='%SCHEMA%%SCHEMA_VERSION%'; set @METHOD='%YADAMU_PARSER%'" <%YADAMU_SQL_PATH%\RECREATE_SCHEMA.sql >>%YADAMU_LOG_PATH%\RECREATE_SCHEMA.log
call %YADAMU_BIN%\upload.bat --RDBMS=%YADAMU_VENDOR% --USERNAME=%DB_USER% --HOSTNAME=%DB_HOST% --PORT=%DB_PORT% --PASSWORD=%DB_PWD% --DATABASE=%DB_DBNAME%  FILE=%YADAMU_OUTPUT_PATH%\%FILENAME%%PRIOR_VERSION%.json TO_USER=\"%SCHEMA%%SCHEMA_VERSION%\"  MODE=%MODE% LOG_FILE=%YADAMU_IMPORT_LOG% EXCEPTION_FOLDER=%YADAMU_LOG_PATH%
