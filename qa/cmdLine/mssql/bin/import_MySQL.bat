call qa\bin\initialize.bat %1 %~dp0 mysql import
...
node %YADAMU_BIN%\import --rdbms=%YADAMU_DB% --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%SCHEMA%%SCHEMAVER% file=%YADAMU_EXPORT_PATH%\%FILENAME%.json to_user=\"dbo\" mode=%MODE%  log_file=%YADAMU_IMPORT_LOG%
