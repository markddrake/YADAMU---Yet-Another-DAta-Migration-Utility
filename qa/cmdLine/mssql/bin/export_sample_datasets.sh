...
source $YADAMU_BIN/export.sh --RDBMS=$YADAMU_VENDOR --USERNAME=$DB_USER --HOSTNAME=$DB_HOST --PASSWORD=$DB_PWD --DATABASE=$SCHEMA FILE=$YADAMU_EXPORT_PATH/$FILENAME.json overwrite=yes FROM_USER=\"dbo\" MODE=$MODE LOG_FILE=$YADAMU_EXPORT_LOG  EXCEPTION_FOLDER=$YADAMU_LOG_PATH
END_TIME=`date +%s`
