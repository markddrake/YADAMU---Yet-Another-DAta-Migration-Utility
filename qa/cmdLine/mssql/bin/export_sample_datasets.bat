call qa\bin\initialize.bat $1 $BASH_SOURCE[0] mssql export
...
node %YADAMU_BIN%\import --rdbms=%YADAMU_DB% --username=%DB_USER% --hostname=%DB_HOST% --password=%DB_PWD% --database=%SCHEMA% file=%YADAMU_EXPORT_PATH%\%FILENAME%.json overwrite=yes from_user=\"dbo\" mode=%MODE% log_file=%YADAMU_EXPORT_LOG%