...
call %YADAMU_BIN%\export.bat --RDBMS=%YADAMU_VENDOR% --USERNAME=%DB_USER% --HOSTNAME=%DB_HOST% --PASSWORD=%DB_PWD% --DATABASE=%SCHEMA%  FILE=%YADAMU_EXPORT_PATH%\%FILENAME%.json  overwrite=true ENCRYPTION=false FROM_USER=\"dbo\"  MODE=%MODE% LOG_FILE=%YADAMU_EXPORT_LOG%  EXCEPTION_FOLDER=%YADAMU_LOG_PATH%%
