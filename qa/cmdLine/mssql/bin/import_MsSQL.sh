...
source $YADAMU_BIN/import.sh --RDBMS=$YADAMU_VENDOR --USERNAME=$DB_USER --HOSTNAME=$DB_HOST --PASSWORD=$DB_PWD --DATABASE=$SCHEMA$SCHEMA_VERSION FILE=$YADAMU_EXPORT_PATH/$FILENAME.json ENCRYPTION=false TO_USER=\"dbo\" LOG_FILE=$YADAMU_IMPORT_LOG  EXCEPTION_FOLDER=$YADAMU_LOG_PATH
...
export MSSQL_SCHEMA="$SCHEMA$SCHEMA_VERSION"
sqlcmd -U$DB_USER -P$DB_PWD -S$DB_HOST -d$DB_DBNAME -I -e -i$YADAMU_SQL_PATH/RECREATE_SCHEMA.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
source $YADAMU_BIN/import.sh --RDBMS=$YADAMU_VENDOR --USERNAME=$DB_USER --HOSTNAME=$DB_HOST --PASSWORD=$DB_PWD --DATABASE=$SCHEMA$SCHEMA_VERSION FILE=$YADAMU_OUTPUT_PATH/$FILENAME$PRIOR_VERSION.json ENCRYPTION=false TO_USER=\"dbo\" LOG_FILE=$YADAMU_IMPORT_LOG  EXCEPTION_FOLDER=$YADAMU_LOG_PATH
export ID1=$PRIOR_VERSION
export ID2=$SCHEMA_VERSION
export METHOD=$YADAMU_PARSER
export DATETIME_PRECISION=9
