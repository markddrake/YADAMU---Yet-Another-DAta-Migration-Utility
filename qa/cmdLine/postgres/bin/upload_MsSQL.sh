source qa/bin/initialize.sh $BASH_SOURCE[0] $BASH_SOURCE[0] mssql upload
...
source $YADAMU_SCRIPT_PATH/upload_MsSQL.sh $YADAMU_INPUT_MSSQL $SCHEMAVER ""
source $YADAMU_SCRIPT_PATH/export_MsSQL.sh $YADAMU_OUTPUT_PATH $SCHEMAVER $SCHEMAVER
export SCHEMAVER=2
psql -U $DB_USER -d $DB_DBNAME -h $DB_HOST -a -vID=$SCHEMAVER -vMETHOD=$YADAMU_PARSER -f $YADAMU_SQL_PATH/RECREATE_MSSQL_ALL.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
source $YADAMU_SCRIPT_PATH/upload_MsSQL.sh $YADAMU_OUTPUT_PATH $SCHEMAVER 1
...
node $YADAMU_BIN/upload --rdbms=$YADAMU_DB --username=$DB_USER --hostname=$DB_HOST --password=$DB_PWD --database=$DB_DBNAME file=$YADAMU_INPUT_MSSQL/$FILENAME.json to_user=\"$SCHEMA$SCHEMAVER\" log_file=$YADAMU_IMPORT_LOG
