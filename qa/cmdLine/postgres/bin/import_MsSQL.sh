...
source $YADAMU_BIN/import_operations_MsSQL.sh $YADAMU_IMPORT_MSSQL $SCHEMA_VERSION ""
...
psql -U $DB_USER -d $DB_DBNAME -h $DB_HOST -a -vID=$SCHEMA_VERSION -vMETHOD=$YADAMU_PARSER -f $YADAMU_SQL_PATH/RECREATE_MSSQL_ALL.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
source $YADAMU_BIN/import_operations_MsSQL.sh $YADAMU_OUTPUT_PATH $SCHEMA_VERSION $PRIOR_VERSION
...
export SCHEMA_VERSION=1
psql -U $DB_USER -d $DB_DBNAME -h $DB_HOST -a -vSCHEMA=$SCHEMA -vID=$SCHEMA_VERSION -vMETHOD=$YADAMU_PARSER -f $YADAMU_SQL_PATH/RECREATE_SCHEMA.sql >>$YADAMU_LOG_PATH/RECREATE_SCHEMA.log
source $YADAMU_BIN/import.sh --RDBMS=$YADAMU_VENDOR --USERNAME=$DB_USER --HOSTNAME=$DB_HOST --PASSWORD=$DB_PWD --DATABASE=$DB_DBNAME FILE=$YADAMU_IMPORT_MSSQL/$FILENAME.json TO_USER=\"$SCHEMA$SCHEMA_VERSION\" LOG_FILE=$YADAMU_IMPORT_LOG  EXCEPTION_FOLDER=$YADAMU_LOG_PATH
