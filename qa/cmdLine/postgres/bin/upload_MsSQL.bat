...
call %YADAMU_BIN%\upload.bat --RDBMS=%YADAMU_VENDOR% --USERNAME=%DB_USER% --HOSTNAME=%DB_HOST% --PASSWORD=%DB_PWD% --DATABASE=%DB_DBNAME%  FILE=%YADAMU_IMPORT_MSSQL%\%FILENAME%.json TO_USER=\"%SCHEMA%%SCHEMA_VERSION%\" LOG_FILE=%YADAMU_IMPORT_LOG% EXCEPTION_FOLDER=%YADAMU_LOG_PATH%
...
set /a SCHEMA_VERSION+=1
psql -U %DB_USER% -d %DB_DBNAME% -h %DB_HOST% -a -vSCHEMA=%SCHEMA% -vID=%SCHEMA_VERSION% -vMETHOD=%YADAMU_PARSER% -f %YADAMU_SQL_PATH%\RECREATE_SCHEMA.sql >>%YADAMU_LOG_PATH%\RECREATE_SCHEMA.log
call %YADAMU_BIN%\upload.bat --RDBMS=%YADAMU_VENDOR% --USERNAME=%DB_USER% --HOSTNAME=%DB_HOST% --PASSWORD=%DB_PWD% --DATABASE=%DB_DBNAME%  FILE=%YADAMU_OUTPUT_PATH%\%FILENAME%%PRIOR_VERSION%.json TO_USER=\"%SCHEMA%%SCHEMA_VERSION%\" LOG_FILE=%YADAMU_IMPORT_LOG% EXCEPTION_FOLDER=%YADAMU_LOG_PATH%
